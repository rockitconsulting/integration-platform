...
public class CronManagerUtil {
	...
	public CronManagerUtil(String expression) {
	...
	public CronManagerUtil(String expression, TimeZone timeZone) {
	...
	private CronManagerUtil(String expression, String[] fields) {
	...
	private void doNext(Calendar calendar, int dot) {
		...
		int updateSecond = findNext(this.seconds, second, calendar,
				Calendar.SECOND, Calendar.MINUTE, emptyList);
		...
		int updateMinute = findNext(this.minutes, minute, calendar,
				Calendar.MINUTE, Calendar.HOUR_OF_DAY, resets);
		...
		int updateHour = findNext(this.hours, hour, calendar,
				Calendar.HOUR_OF_DAY, Calendar.DAY_OF_WEEK, resets);
		...
		int updateDayOfMonth = findNextDay(calendar, this.daysOfMonth,
				dayOfMonth, daysOfWeek, dayOfWeek, resets);
		...
		int updateMonth = findNext(this.months, month, calendar,
				Calendar.MONTH, Calendar.YEAR, resets);
	...
	private int findNextDay(Calendar calendar, BitSet daysOfMonth,
			int dayOfMonth, BitSet daysOfWeek, int dayOfWeek,
		...
		while ((!daysOfMonth.get(dayOfMonth) || !daysOfWeek.get(dayOfWeek - 1))
				&& count++ < max) {
	...
	private int findNext(BitSet bits, int value, Calendar calendar, int field,
			int nextField, List<Integer> lowerOrders) {
	...
	private int[] getRange(String field, int min, int max) {
		...
		if (result[0] < min || result[1] < min) {
			...
			throw new IllegalArgumentException("Range less than minimum ("
					+ min + "): '" +
		...
		if (result[0] > result[1]) {
			...
			throw new IllegalArgumentException("Invalid inverted range: '"
					+ field +
	...
	public static boolean isValidExpression(String expression) {
		...
		try {
			...
			new CronManagerUtil(expression, fields);
	...
	public boolean equals(Object other) {
		...
		if (!(other instanceof CronManagerUtil)) {

			return false;

		}

		CronManagerUtil otherCron = (CronManagerUtil) other;
		...
		return (this.months.equals(otherCron.months)
				...
				this.minutes.equals(otherCron.minutes) && this.seconds
					.equals(otherCron.seconds));
	...
	public int hashCode() {
		...
		return (17 * this.months.hashCode() + 29 * this.daysOfMonth.hashCode()
				+ 37 * this.daysOfWeek.hashCode() +

				41 * this.hours.hashCode() + 53 * this.minutes.hashCode() + 61 * this.seconds
				.hashCode());
